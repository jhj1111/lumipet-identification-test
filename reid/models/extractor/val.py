...
class ExtractorValidator(BaseValidator):
    ...
    def validate(self, data=None, **kwargs):
        """
        kwargs['gallery']: [(img_path, label), ...] -> Images used to build the temporary DB.
        kwargs['query']: [(img_path, label), ...] -> Images used to test accuracy.
        kwargs['pipeline']: The ReIdModel instance to use for prediction.
        """
        gallery_data = kwargs.get('gallery')
        query_data = kwargs.get('query')
